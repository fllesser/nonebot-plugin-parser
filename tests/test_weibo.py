...
async def test_weibo_pics():
    ...
    async def test_parse_share_url(url: str) -> None:
        ...
        if video_info.video_url:
            await stream_downloader.download_video(video_info.video_url, ext_headers=ext_headers)
            logger.success(f"{url} | 微博视频下载成功")
        if video_info.pic_urls:
            files = await stream_downloader.download_imgs_without_raise(video_info.pic_urls, ext_headers=ext_headers)
