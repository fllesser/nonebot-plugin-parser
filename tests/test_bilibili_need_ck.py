...
async def test_bilibili_favlist():
    ...
    files = await stream_downloader.download_imgs_without_raise(urls)
...
async def test_encode_h264_video():
    ...
    v_path, a_path = await asyncio.gather(
        stream_downloader.download_file_by_stream(video_url, file_name=f"{bvid}-video.m4s", ext_headers=HEADERS),
        stream_downloader.download_file_by_stream(audio_url, file_name=f"{bvid}-audio.m4s", ext_headers=HEADERS),
    )
