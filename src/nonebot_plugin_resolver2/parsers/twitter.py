...
class TwitterParser:
    @classmethod
    async def parse_x_url_new(cls, x_url: str):
        ...
        resp = await req_xdown_api(x_url)
        ...
        if html_content is None:
            raise ParseException("解析失败, data为空")
        ...
        if len(pic_urls) != 0:
            return ImageContent(pic_urls=pic_urls)

    @classmethod
    def regex_template(cls, flag: str) -> re.Pattern[str]:
        """
        根据标志生成正则表达式模板
        """
    ...
    async def get_first_video_url(cls, html_content: str) -> str | None:
        ...
        """
        # 匹配第一个视频下载链接
        matched = re.search(cls.regex_template(" MP4"), html_content)
    ...
    async def get_all_pic_urls(cls, html_content: str) -> list[str]:
        ...
        提取所有图片链接
        """
        return re.findall(cls.regex_template("图片"), html_content)
    ...
    async def get_all_gif_urls(cls, html_content: str) -> list[str]:
        ...
        提取所有 GIF 链接
        """
        return re.findall(cls.regex_template(" gif"), html_content)
